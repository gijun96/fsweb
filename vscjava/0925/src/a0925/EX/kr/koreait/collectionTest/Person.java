...
//	TreeSet에 저장할 클래스는 Comparable 인터페이스를 구현받고 compareTo() 메소드를 Override 한 후 객체 정렬 기준을 지정한다.
//	TreeSet에 저장할 클래스에 구현한 Comparable 인터페이스의 제네릭에는 현재 클래스의 이름을 쓰면된다.
...
//	compareTo() 메소드는 자신(this)과 인수(Person o)로 넘어온 객체(add() 메소드의 인수로 지정된 객체)에 저장된 데이터를 비교한다.
