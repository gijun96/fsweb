...
//		TreeSet은 HashSet과 다르게 데이터가 입력되는 순서와 관계 없이 정렬되서 저장된다.
...
//		remove(value) => TreeSet에 저장된 value를 제거한다.
		tset.remove(500);
		System.out.println(tset.size() + " : " + tset);
//		clear() => TreeSet에 저장된 모든 value를 제거한다.
		tset.clear();
		System.out.println(tset.size() + " : " + tset);
		System.out.println("==============================================");
...
//		로또 1등 번호
		...
		while(true) {
			...
			tset.add(lotto);
//			TreeSet 객체에 중복되지 않는 숫자 6개가 저장되면 반복을 탈출한다.
			if(tset.size() == 6) {
				break;
			}
		}
...
//		tset.remove(bonus);
		...
		while(true) {
			bonus = random.nextInt(45) + 1;
//			contains() => TreeSet 객체에 인수로 지정된 데이터가 있으면 true, 없으면 false를 리턴한다.
