...
class Shape {
//	final을 멤버 변수 선언시 앞에 붙여주면 그 변수는 프로그램에서 값을 변경시킬 수 없다. => 상수 => 일반적으로 대문자로 만들어 사용한다.
...
//	인터페이스란 클래스의 특별한 형태로 무조건 정적(static final) 멤버 변수(상수)와 추상 메소드로만 구성된다.
interface Draw {
	public static final double PI = 3.141592653589793;		// 정적 멤버 변수, 상수
//	인터페이스는 변수 선언시 앞의 내용을 생략하면 컴파일러가 자동으로 public static final을 붙여준다.
	int LIMIT = 1000;
	public abstract void move1();	// 추상 메소드
//	인터페이스는 메소드 선언시 앞의 내용을 생략하면 컴파일러가 자동으로 public abstract을 붙여준다.
	void erase();
...
//		static으로 선언된 변수나 메소드는 클래스 객체를 생성하지 않고 클래스 이름에 "."을 찍어서 바로 접근할 수 있다.
		System.out.println("Math.PI = " + Math.PI);
//		final 변수(상수)의 값을 프로그램에서 변경하려 했으므로 에러가 발행된다.
