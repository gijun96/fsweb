package a0925.EX.kr.koreait.interfaceTest;
...
//	상수를 초기화 시키는 방법은 2가지가 있는데 1번째 방법은 상수 선언시 "="를 사용해서 초기화 시키는 방법이고 2번째 방법은 객체가 생성될 때
//	생성자에서 초기화 시키는 방법이 있다.
	final int MAX_HP;		// 최대 HP, 상수
	int hp;					// 현재 HP
	
//	모든 유닛은 종류에 따라서 서로 다른 최대 HP를 가져야 하므로 기본 생성자를 사용해서 최대 HP를 초기화 시킬 수 없고 생성자의 인수로 상수를 초기화
//	시킬 값을 넘겨받아서 상수를 초기화 시킨다.
//	기본 생성자를 별도로 정의하지 않았고 Unit(int hp) 생성자를 만들어 사용했기 때문에 자바 컴파일러가 기본 생성자를 자동으로 만들지 않는다.
	public Unit(int hp) {
//		④ Unit 클래스 생성자에서 넘어온 150을 hp에 받아서 최대 hp(MAX_HP)를 초기화 시킨다.
		MAX_HP = hp;
//		⑤ Unit 클래스 생성자가 종료되면 Unit 클래스의 생성자를 호출한 곳으로 돌아간다.
	}
}
...
//		③ GroundUnit 클래스 생성자에서 넘어온 150을 hp에 받아서 hp(150)을 인수로 부모 클래스(Unit)의 생성자 Unit(int hp)를 호출한다.
		super(hp);
//		⑥ GroundUnit 클래스 생성자가 종료되면 GroundUnit 클래스의 생성자를 호출한 곳으로 돌아간다.
	}
}
...
//		② Tank 클래스의 기본 생성자가 실행되면 150을 인수로 부모 클래스(GroundUnit)의 생성자 GroundUnit(int hp)를 호출한다.
		super(150);
//		⑦ Tank의 MAX_HP가 150으로 초기화되었으므로 현재 hp를 MAX_HP로 초기화시킨다.
		hp = MAX_HP;
...
//	오퍼랜드(메소드가 처리할 데이터) 오버로딩
//	메소드의 인수로 클래스 또는 인터페이스의 객체를 지정하면 인수로 지정된 클래스를 상속받거나 인터페이스를 구현받은 모든 클래스 또는 인터페이스
//	객체를 인수로 받을 수 있다.
	public void repair(Repairble repairble) {
		
//		표시 인터페이스 타입으로 수리할 객체를 받았지만 표시 인터페이스에는 아무런 내용이 코딩되어있지 않기 때문에 인수로 넘겨받은 객체를 실제로
//		구현한 내용이 들어있는 클래스로 형변환을 시킨 후 사용해야 한다.
