...
//	GridLayout은 컨테이너를 격자(grid, 바둑판) 모양으로 나누고 컴포넌트를 나눠진 구역에 순서대로 배치하는 레이아웃 매니저이다.
public class GridLayoutTest extends Frame {

//	버튼 배열만 선언된 상태로 실제 프로그램에서 사용할 버튼 객체는 아직 생성되지 않은 상태이다.
//	버튼 배열을 생성한 후 각각의 버튼 배열 요소에 버튼 객체를 생성하지 않은 상태로 사용하면 NullPointerException이 발생된다.
...
//		new GridLayout(4, 4, 3, 0) => 세로 여백이 3인 4행 4열 GridLayout
//		new GridLayout(4, 4, 0, 3) => 가로 여백이 3인 4행 4열 GridLayout
//		new GridLayout(4, 4, 3, 3) => 모든 여백이 3인 4행 4열 GridLayout
		
//		numbers 배열에 저장된 숫자를 섞는다.
...
//			buttons[i] = new Button(String.format("%d", i + 1));
			...
			/*
//			버튼 위에 글씨가 있을 때
//			getActionCommand() : 버튼 위의 명령(문자열)을 얻어온다.
			if(buttons[i].getActionCommand().equals("16")) {
//				setEnabled(false) : 컴포넌트를 비활성화 시킨다.
...
//			버튼 위에 글씨가 없을 때
//			setName() : 컴포넌트에 이름을 지정한다. getName() : 컴포넌트에 이름을 얻어온다.
