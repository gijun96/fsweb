...
//		1. JComboBox 클래스로 콤보 박스를 만들려면 콤보 박스에 표시할 목록을 저장한 배열을 만들고 배열을 JComboBox 클래스 생성자의 인수로
//		넘겨주면 된다.
		String[] fruit = {"========== 뭐먹을래 ==========", "黷바나나", "파인애플", "망고", "두리안", "베리베리스트로베리"};
		comboBox = new JComboBox<String>(fruit);
		
//		2. 기본 생성자로 JComboBox 클래스 객체를 만들고 setModel() 메소드로 DefaultComboBoxModel 클래스 객체에 콤보 박스에 표시할 목록이 저장된
//		배열을 인수로 넘겨서 만든다.
//		comboBox = new JComboBox<String>();
//		comboBox.setModel(new DefaultComboBoxModel<String>(new String[] {"========== 뭐먹을래 ==========", "黷바나나", "파인애플", "망고", 
//				"두리안", "베리베리스트로베리"}));
...
//				getItemAt(index) : JComboBox 클래스로 만든 콤보 박스의 index 번째 항목에 해당되는 아이템을 얻어온다.
...
//					removeItemAt(anIndex) : JComboBox 클래스로 만든 콤보 박스에서 anIndex 번째 아이템을 제거한다.
			...
			case "추가":
				...
				if(item.length() > 0) {
//					addItem() : JComboBox 클래스로 만든 콤보 박스에 데이터를 추가한다.
