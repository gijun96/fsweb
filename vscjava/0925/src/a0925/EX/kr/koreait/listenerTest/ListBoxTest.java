...
//		윈도우 중단의 리스트 박스 세팅
//		단일 선택 리스트 박스
//		new List() : 선택할 수 있는 아이템의 개수가 기본값(4개)으로 보여지는 리스트 박스를 만든다.
//		new List(아이템 개수) : 생성자의 인수로 넘겨준 아이템의 개수 만큼 목록이 보여지는 리스트 박스를 만든다.
		singleList = new List();		// 단일 선택 리스트 박스를 만든다.
		singleList.add("고구마");		// 리스트 박스에 아이템을 추가한다.
		singleList.add("감자");
		singleList.add("옥수수");
		singleList.add("오이");
		singleList.add("호박");
//		다중 선택 리스트 박스
//		new List(아이템 개수, 목록 선택 방법) : 생성자의 인수로 넘겨준 아이템의 개수 만큼 목록이 보여지는 리스트 박스를 만들고 목록 선택 방법을
//		생략하면 false가 기본값이고 단일 목록 선택 리스트 박스가 된다. => 목록 선택 방법에 true를 쓰면 다중 목록 선택 리스트 박스가 된다.
...
//				getSelectedItems() : 다중 선택 리스트 박스에서 선택된 아이템의 값들을 배열로 받아온다.
...
//				다중 선택 리스트 박스에서 삭제할 아이템 목록의 위치를 얻어와서 배열에 넣어준다.
				int[] deletList = multiList.getSelectedIndexes();
...
//					int delIndex = 0;
//					for(int i = 0; i < deletList.length; i++) {
//						multiList.remove(deletList[i] - delIndex++);
//					}
