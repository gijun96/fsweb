...
//				getSelectedValuesList() : JList 클래스로 만든 다중 목록 리스트 박스에서 선택항 항목의 아이템들을 얻어온다. => 선택한 아이템들을
//				List 인터페이스 타입으로 얻어오기 때문에 ArrayList로 형변환 시켜서 받아준다.
...
//					JList에 데이터를 추가할 때 DefaultListModel 클래스 객체에 addElement() 메소드를 사용해 아이템을 추가했던 것 처럼 데이터 
//					아이템을 삭제할 때도 DefaultListModel 클래스 객체에서 removeElementAt() 메소드로 삭제한다.
