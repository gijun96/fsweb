...
public class PuzzleGameActionListener2 extends JFrame implements ActionListener {
...
//	이미지 퍼즐에 사용할 이미지를 기억할 Image 배열을 만든다.
	...
	public PuzzleGameActionListener2() {
		setTitle("PuzzleGame");
...
//			}
//			버튼에 이미지를 올리면 getActionCommand() 메소드를 사용할 수 없으므로 setName() 메소드로 버튼에 이름을 붙여서 처리한다.
	...
	public static void main(String[] args) {
		...
		PuzzleGameActionListener2 window = new PuzzleGameActionListener2();
...
//			ActionEvent 클래스 타입의 변수 e에는 ActionListener가 실행된 ActionCommand는 넘어오지만 버튼에 할당된 이름은 넘어오지 않기 때문에
//			Object 타입의 객체로 ActionListener가 실행된 버튼 정보를 받아 형변환 시킨 후 이름을 얻어내서 사용해야 한다.
...
//				퍼즐 버튼에 ActionCommand가 없기 때문에 getActionCommand() 메소드를 실행하면 공백을 얻어오게 되고 공백은 parseInt() 메소드를
//				실행하면 NumberFormatException이 발생되기 때문에 퍼즐 버튼에 붙여준 이름을 getName() 메소드로 얻어와 비교해야 한다.
