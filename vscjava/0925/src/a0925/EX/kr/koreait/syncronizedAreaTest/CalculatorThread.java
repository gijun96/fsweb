...
//	연산을 하는 스레드 클래스, 실행 시간이 긴 스레드 클래스
...
//		currentTimeMillis() 메소드를 사용해 얻어온 시간을 연산하면 연산 결과에 offset이 포함된다.
//		단순 연산시에는 별 문제가 되지 않지만 SimpleDateFormat 클래스 객체를 사용해 서식을 지정할 경우 9시간이 더 나오는 문제가
//		발생된다. => offset은 세계 시간의 기준이 되는 그리니치(영국) 시간을 기준으로 currentTimeMillis() 메소드를 실행한 지역과
...
//			notifyAll() : wait() 메소드로 일시적으로 중지한 모든 스레드를 다시 실행시킨다.
			shareArea.notify();
		}
