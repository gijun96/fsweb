...
//		동기화란 작업의 순서를 정한다는 의미로 공유 영역을 사용하는 스레드가 실행중일 때 같은 공유 영역을 사용하는 다른 스레드가
...
//		동기화 블록에는 실행중 절대로 작업 전환이 이루어지면 안되는 내용을 코딩한다.
		...
		synchronized (shareArea) {
			...
			for(int i = 0; i < 20; i++) {
				shareArea.lee.deposit(1000000);					// 이몽룡 계좌 100만원 입금
				System.out.println("이몽룡 계좌 100만원 입금");
				if(shareArea.hong.withDraw(1000000) != 0) {		// 홍길동 계좌 100만원 출금
